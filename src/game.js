...
const game={name:'',shells:new Set(),notes:new Set(),paniers:new Set(),quests:{shells:0,cat:0,crabs:0,bread:0,treasure:0,fish:0,notes:0,goats:0,oil:0,farm:0,hens:0,market:0,bouquet:0,laine:0,farine:0,paniers:0,truites:0},goatsCaught:0,oil:false,flour:false,trout:0,coins:25,seeds:{tomate:3},produce:{},plots:{},harvested:0,seedSel:0,unlocked:[],farmLevel:0,shorn:{},log:[],discovered:{},playtime:0,clock:DAY*.1,catFollow:false,crabsCaught:0,fish:0,delivered:{},dug:false,fireworksUntil:-1,finale:false,island:'azura'};
let playing=false,dialog=null,dirty=false,lastSave=0,minigame=null;
const NAMES={tomas:'Tomas',anae:'Anaé',lila:'Lila',oro:'Oro',pia:'Pia',marco:'Marco',bastien:'Bastien',cat:'Pistache',gardien:'Elio',marchand:'Basile',sibylle:'Sibylle',nino:'Nino',margot:'Margot'};
// Maillages provisoires des PNJ des îles de la phase 3 (Astra livrera les définitifs dans rig.js sous les mêmes clés).
npcMeshes.sibylle=npcMeshes.sibylle||meshDyn(humanoid({shirt:color(0x7a5ba6),pants:color(0xe8dcc8),skin:C.skin[0],hair:color(0x8a5a2e),longHair:true,scale:.97}));
npcMeshes.nino=npcMeshes.nino||meshDyn(humanoid({shirt:color(0xe76f51),pants:color(0x2a9d8f),skin:C.skin[2],hair:color(0x1e1410),hat:'cap',hatColor:color(0x2a9d8f),scale:.68}));
...
{id:'gardien',name:'Elio, le gardien du phare',x:33.2,z:-23.2,leash:1.0,speed:.8},
{id:'marchand',name:'Basile, le marchand',x:-23.6,z:8.9,leash:.35,speed:.7,scale:1},
{id:'sibylle',name:'Sibylle, la châtelaine',x:32.4,z:38.2,leash:.9,speed:.8,scale:.97},
{id:'nino',name:'Nino',x:-30.6,z:29.2,leash:.45,speed:1.8,scale:.68},
{id:'margot',name:'Margot, la meunière',x:2.7,z:31.3,leash:.5,speed:.9,scale:.94}];
...
const places=[{id:'village',name:'Le village',x:-2.5,z:3,r:3.5,minY:3},{id:'port',name:'La crique',x:-4.2,z:14.5,r:2.4,minY:0},{id:'summit',name:'Le belvédère',x:1.7,z:-5,r:2.5,minY:9.4},{id:'phare',name:"La crique du Phare",x:35.5,z:-8,r:4,minY:.3},{id:'champs',name:"L'Île des Champs",x:-32,z:10,r:5,minY:1},{id:'arche',name:"Le manoir de l'Arche",x:33,z:38,r:3,minY:12},{id:'cabanes',name:'Les cabanes perchées',x:-32,z:28.5,r:2.6,minY:9.4},{id:'gorge',name:'Le pont de la Gorge',x:0,z:52,r:2.8,minY:8.5}];
const panierSpots=[[-25.2,36.2],[-30.9,48.9],[-34.2,35.6]].map((p,i)=>{const [x,z]=snap(p[0],p[1]);return{id:i,x,z,y:cellH(x,z)};});
const gorgeFishSpot=(()=>{const [x,z]=snap(3.1,32.4);return{x,z,y:cellH(x,z)};})();
...
const goats=[[37.9,-12.4,1],[32.2,-14.8,.85],[36.5,-19.3,.95]].map((p,i)=>{const [x,z]=snap(p[0],p[1]);return{id:i,x,z,y:cellH(x,z),heading:Math.random()*TAU,phase:0,amp:0,speed:0,scale:typeof goatMesh!=='undefined'||npcMeshes.goat?p[2]:1.7,state:'idle',t:Math.random()*3,home:[x,z],bones:new Float32Array(16*8),hidden:false,goat:true,pose:'graze',leash:1.6};});
const lanterns=[[-3.3,10.8],[-4.6,15.6],[-2.0,3.5],[4.3,4.1],[1.4,-.1],[2.4,-4.1],[5.0,10.3],[-5.6,8.9],[-8.6,3.0],[-33.5,3.9],[-22.6,11.2]].map(p=>{const [x,z]=snap(p[0],p[1],2);return{x,z,y:cellH(x,z),bones:new Float32Array(16)};});
...
// ---------- sauvegarde ----------
function saveGame(silent){const d={v:3,name:game.name,x:player.x,z:player.z,heading:player.heading,cam:{theta:goal.theta,phi:goal.phi,r:goal.r},shells:[...game.shells],notes:[...game.notes],paniers:[...game.paniers],quests:game.quests,log:game.log,discovered:game.discovered,playtime:game.playtime,clock:game.clock,catFollow:game.catFollow,crabsCaught:game.crabsCaught,fish:game.fish,delivered:game.delivered,dug:game.dug,finale:game.finale,island:game.island,goatsCaught:game.goatsCaught,oil:game.oil,flour:game.flour,trout:game.trout,farm:farmSaveData(),cat:[cat.x,cat.z],said:Object.fromEntries(npcs.map(n=>[n.id,n.said])),savedAt:Date.now()};
...
hens:()=>game.eggsTotal>=3?"Le poulailler tourne : vends les œufs à Pia.":game.hens?`Nourris tes poules avec du blé au poulailler de l'Île des Champs, puis ramasse les œufs le lendemain (${game.eggsTotal||0}/3 œufs).`:"Achète une poule chez Anaé.",
...
market:()=>`Apporte à Basile, à l'étal des Champs, 3 tomates et 3 blés (${Math.min(3,game.produce.tomate||0)}/3 🍅, ${Math.min(3,game.produce.ble||0)}/3 🌾).`,
bouquet:()=>`Pia, à Azura, voudrait 3 fleurs de sable de ton champ pour son comptoir (${Math.min(3,game.produce.fleur||0)}/3 🌸).`,
laine:()=>`Anaé tricote une écharpe : apporte-lui 2 pelotes de laine des moutons des Champs (${Math.min(2,game.produce.laine||0)}/2 🧶). Tonds-les avec E.`,
...
farine:()=>game.flour?"Rapporte le sac de farine à Sibylle, au manoir de l'Arche.":`Porte 3 blés de ton champ à Margot, au moulin de la Gorge : elle moudra la farine de Sibylle (${Math.min(3,game.produce.ble||0)}/3 🌾).`,
paniers:()=>game.paniers.size>=3?"Rapporte les paniers à Nino, sur la plateforme de l'Île aux Cabanes.":`Retrouve les 3 paniers de Nino (${game.paniers.size}/3) : un sur l'îlot est, un sur la plage sud, un au sommet du piton.`,
truites:()=>game.trout>=2?"Rapporte les 2 truites à Margot, au moulin de la Gorge.":`Pêche 2 truites au bassin de la chute, depuis le socle du moulin de la Gorge (${game.trout}/2).`};
...
const q=game.quests;if(q.shells===0)return"Explore l'île et parle aux habitants. Tomas t'attend sur le ponton.";const todo=[['cat','Anaé, dans le village, a besoin d\'aide.'],['crabs','Lila, sur la plage, a une idée de jeu.'],['bread','Pia, près du ponton, sort son pain du four.'],['treasure','Marco, sur la plage, a trouvé quelque chose.'],['fish','Reparle à Tomas : il a une canne à te prêter.'],['notes','Bastien joue de la guitare sur la terrasse du haut. Va l\'écouter.'],['goats','Prends le voilier de Tomas : Elio, le gardien du phare, a besoin d\'aide au sommet de son île.'],['farm','Anaé a une parcelle à te confier près de la maison de Pia. Va la voir.'],['hens','Anaé vend des poules : installe-les au poulailler de l\'Île des Champs.'],['market','Basile, à l\'étal des Champs, a une commande pour toi.'],['bouquet','Pia aimerait des fleurs de ton champ. Va la voir à Azura.'],['laine','Anaé a besoin de laine : parle-lui au village.'],['paniers','Nino, sur les plateformes de l\'Île aux Cabanes, a perdu quelque chose.'],['farine','Sibylle t\'attend au manoir de l\'Arche : une fête se prépare.'],['truites','Margot, au moulin de la Gorge, aurait besoin de ta canne.']].filter(([k])=>q[k]===0&&(['hens','market','farine'].includes(k)?q.farm>=1:['bouquet','laine'].includes(k)?q.market===2:k==='truites'?q.fish>=1:true));
...
case'anae':if(q.laine===0&&q.market===2)return wrap({lines:["Basile dit qu'il y a des moutons sur l'Île des Champs ! L'hiver arrive et je tricote une écharpe pour Oro.","Tonds-les et rapporte-moi 2 pelotes de laine, je te donne 30 pièces et ma reconnaissance."],end:()=>startQuest('laine')});
if(q.laine===1&&(game.produce.laine||0)>=2)return wrap({lines:["Deux pelotes, douces comme un nuage ! Oro aura chaud cet hiver.","Tiens, 30 pièces. Tu es devenu un vrai fermier de l'archipel."],end:()=>{game.produce.laine-=2;game.coins+=30;sfx('coin');finishQuest('laine','L\'écharpe d\'Anaé');}});
if(q.farm===0&&q.cat>=1)return wrap({lines:[`${me}, tu as l'air de savoir te servir de tes mains. Il y a un coin de sable plat à l'ouest de la plage, entre le ponton et la maison de Pia.`,"Je te donne trois graines de tomate pour commencer. Bêche, sème, arrose chaque matin, et reviens m'acheter des graines quand tu auras vendu ta récolte à Pia."],end:()=>{startQuest('farm');game.seeds.tomate=(game.seeds.tomate||0)+3;refreshHUD();}});
...
case'marchand':if(q.farm===0)return wrap({lines:[`Bienvenue sur l'Île des Champs, ${me} ! Je tiens l'étal du marché.`,"Va d'abord voir Anaé à Azura : elle te confiera ta première parcelle. Ensuite je t'achèterai tes récoltes et je te vendrai des graines ici même."]});
if(q.market===0)return wrap({lines:["Bienvenue à l'étal ! J'achète tes récoltes et tes œufs, et je vends les mêmes graines qu'Anaé, sans traverser la mer.","Une commande pour commencer : le voilier de Tomas emporte des paniers vers les autres îles. Apporte-moi 3 tomates et 3 blés, je te paie 40 pièces en plus du prix."],end:()=>startQuest('market')});
if(q.market===1&&(game.produce.tomate||0)>=3&&(game.produce.ble||0)>=3)return wrap({lines:["Trois tomates, trois blés, tout est là ! Le panier part au prochain voilier.","Tiens, 40 pièces. Et dis à Pia et Anaé, à Azura, que ton champ donne bien : elles auront des idées."],end:()=>{game.produce.tomate-=3;game.produce.ble-=3;game.coins+=40;sfx('coin');finishQuest('market','La première commande de Basile');}});
if(q.market===1)return wrap({lines:["Il me faut 3 tomates et 3 blés pour la commande. Sème, arrose, et reviens me voir !"]});
return wrap({lines:[n.said?"Alors, qu'est-ce qu'on fait aujourd'hui ?":"Bienvenue à l'étal ! J'achète tes récoltes, tes œufs et ta laine, et je vends les graines."],end:()=>openShop('market')});
case'pia':if(q.bouquet===0&&q.market===2)return wrap({lines:["Basile m'a écrit : ton champ des Champs donne déjà ! J'aimerais fleurir mon comptoir.","Apporte-moi 3 fleurs de sable de ta ferme, je te les paie 30 pièces le bouquet, en plus du prix."],end:()=>startQuest('bouquet')});
if(q.bouquet===1&&(game.produce.fleur||0)>=3)return wrap({lines:["Quelles couleurs ! Mon comptoir n'a jamais été aussi joli.","Voilà 30 pièces, et reviens quand tu veux avec ta récolte."],end:()=>{game.produce.fleur-=3;game.coins+=30;sfx('coin');finishQuest('bouquet','Le bouquet de Pia');}});
if(q.farm>=1&&Object.values(game.produce).some(v=>v>0))return wrap({lines:[q.bouquet===1?"Il me faut 3 fleurs de sable pour le bouquet. Sinon, montre-moi ta récolte, j'achète tout au prix juste.":"Tu as récolté ? Montre-moi ça, j'achète tout au prix juste."],end:()=>openShop('sell')});
...
case'sibylle':if(q.farine===0&&q.farm>=1)return wrap({lines:[`Bienvenue au manoir, ${me}. Je suis Sibylle ; ma famille garde l'Arche depuis six générations.`,"Je donne une fête pour tout l'archipel, mais mes fours attendent : plus une once de farine !","Margot, au moulin de la Gorge, moud la meilleure de l'archipel. Porte-lui 3 blés de ton champ et rapporte-moi le sac : je te paierai 45 pièces."],end:()=>startQuest('farine')});
if(q.farine===1&&game.flour)return wrap({lines:["De la farine fraîche du moulin ! Les fours vont ronfler toute la nuit.","Voilà tes 45 pièces. Et considère-toi invité d'honneur de la fête du manoir."],end:()=>{game.flour=false;game.coins+=45;sfx('coin');finishQuest('farine','La fête du manoir');}});
if(q.farine===1)return wrap({lines:["Margot moud la farine au moulin de la Gorge. 3 blés de ton champ, et le tour est joué."]});
if(q.farine===0)return wrap({lines:["Bienvenue au manoir de l'Arche. Peu de visiteurs montent jusqu'au parvis.","On dit qu'un champ se cultive sur l'Île des Champs. Reviens me voir quand tu sauras faire pousser le blé : j'aurai une commande pour toi."]});
return wrap({lines:[pick(["Sous la voûte, le vent chante plus fort que partout ailleurs.","Le grès de l'Arche change de couleur au couchant. Regarde-le depuis le parvis.","Mon grand-père faisait passer son voilier sous la voûte à la voile. Tomas est le seul à oser encore.","La tour de garde n'a plus vu de pirates depuis cent ans. Heureusement."])]});
case'nino':if(q.paniers===0)return wrap({lines:["Oh non, oh non… Le vent a fait valser mes paniers de fruits pendant que je grimpais !","Un est tombé près de la cabane de l'îlot est, un a roulé jusqu'à la plage du sud, et le dernier… tout en haut du grand piton, je crois.","Tu veux bien me les retrouver ? Moi, j'ai les jambes en coton."],end:()=>startQuest('paniers')});
if(q.paniers===1&&game.paniers.size<3)return wrap({lines:[`${game.paniers.size} sur 3. Regarde près de la cabane de l'îlot est, sur la plage sud et au sommet du grand rocher.`]});
if(q.paniers===1)return wrap({lines:["Mes paniers ! Et les fruits n'ont presque rien !","Tiens, 25 pièces. C'est ma tirelire, mais tu les mérites. Reviens quand tu veux, je te montrerai les cabanes."],end:()=>{game.coins+=25;sfx('coin');finishQuest('paniers','Les paniers de Nino');}});
return wrap({lines:[pick(["Un jour, j'aurai ma propre cabane, tout en haut du grand arbre.","Le lagon est chaud comme un bain, le soir. Mais chut, c'est mon coin secret.","Depuis les plateformes, on voit toutes les îles ! Enfin, presque.","Les pirogues ? C'est papa qui les a creusées, avec une herminette."])]});
case'margot':if(q.farine===1&&!game.flour&&(game.produce.ble||0)>=3)return wrap({lines:["Du blé de ton champ ? Voyons ça… La roue tourne, la meule est chaude.","Et voilà : un sac de farine comme on n'en fait plus. Porte-le vite à Sibylle, avant que ses fours refroidissent."],end:()=>{game.produce.ble-=3;game.flour=true;sfx('dig');dirty=true;refreshHUD();}});
if(q.farine===1&&!game.flour)return wrap({lines:["Sibylle veut de la farine ? Apporte-moi 3 blés de ton champ et la meule s'occupe du reste."]});
if(q.truites===0&&q.fish>=1)return wrap({lines:["Tu as une canne, non ? Les truites remontent la chute ces jours-ci : elles sautent presque dans le bassin.","Pêche-m'en deux depuis le socle du moulin, j'en ferai des filets aux amandes. Je te paie 30 pièces."],end:()=>startQuest('truites')});
if(q.truites===1&&game.trout>=2)return wrap({lines:["Deux belles truites ! La poêle est déjà chaude.","Voilà tes 30 pièces. Repasse quand la roue t'appelle."],end:()=>{game.trout=0;game.coins+=30;sfx('coin');finishQuest('truites','Les truites de la Gorge');}});
if(q.truites===1)return wrap({lines:["Le bassin au pied de la chute, depuis le socle du moulin. Deux truites, et la poêle s'occupe du reste."]});
return wrap({lines:[pick(["La roue tourne depuis que la rivière est revenue. Écoute-la : c'est ma musique à moi.","Huit maisons, un moulin, un pont : le village tient tout entier dans la gorge.","La chute chante plus fort au printemps, quand la neige des hauteurs fond.","Monte jusqu'au pont suspendu, tout en haut : la vue vaut le détour."])]});
...
if(game.quests.truites===1&&game.trout<2){const d=Math.hypot(gorgeFishSpot.x-player.x,gorgeFishSpot.z-player.z);if(d<Math.min(bd,1.4)&&Math.abs(gorgeFishSpot.y-player.y)<.6){bd=d;best={type:'fish',trout:true,label:'Pêcher la truite',btn:'Pêcher'};}}
return best;}
...
else if(th.type==='fish')startFishing(th.trout);
else if(th.type==='boat'){if(game.quests.shells<2){toast('Le voilier est à Tomas : rends-lui d\'abord service pour gagner sa confiance.');return;}if(th.dests.length===1)startVoyage(th.dests[0].id);else chooseVoyage(th.dests);}}
function chooseVoyage(dests){shopEl.hidden=false;const box=shopEl.querySelector('div');box.innerHTML='';const h=document.createElement('h2');h.textContent='Où aller ?';box.appendChild(h);const p=document.createElement('p');p.textContent='Tomas hisse la voile. Choisis ta destination.';box.appendChild(p);for(const d of dests){const b=document.createElement('button');b.textContent='⛵ '+d.name;b.onclick=()=>{shopEl.hidden=true;startVoyage(d.id);};box.appendChild(b);}const c=document.createElement('button');c.className='secondary';c.textContent='Rester ici';c.onclick=()=>{shopEl.hidden=true;};box.appendChild(c);}

// ---------- mini-jeu de pêche ----------
const fishingEl=$('#fishing'),cursorEl=fishingEl.querySelector('.cursor'),zoneEl=fishingEl.querySelector('.zone');
function startFishing(trout){const speed=trout?1.5+game.trout*.35:1.1+Math.min(game.fish,6)*.22,zw=trout?.16:Math.max(.14,.26-game.fish*.02),zx=.2+Math.random()*(.8-zw-.2);minigame={pos:0,dir:1,speed,zx,zw,trout:!!trout};zoneEl.style.left=(zx*100)+'%';zoneEl.style.width=(zw*100)+'%';fishingEl.hidden=false;promptEl.hidden=true;actionBtn.textContent='Ferre !';actionBtn.hidden=false;sfx('cast');player.heading=trout?PI:0;}
...
function fishingHit(){const m=minigame;minigame=null;fishingEl.hidden=true;if(m.pos>=m.zx&&m.pos<=m.zx+m.zw){sfx('catch');if(m.trout&&game.quests.truites===1&&game.trout<2){game.trout++;toast(`🐠 Truite ferrée ! ${game.trout}/2`);if(game.trout>=2)setTimeout(()=>toast('Porte-les à Margot, au moulin.'),1600);}else if(!m.trout&&game.quests.fish===1&&game.fish<3){game.fish++;toast(`🐟 Poisson attrapé ! ${game.fish}/3`);if(game.fish>=3)setTimeout(()=>toast('Porte-les à Anaé, dans le village.'),1600);}else toast('🐟 Un beau poisson ! Tu le relâches.');dirty=true;refreshHUD();}else{sfx('fail');toast('Raté ! Il s\'est échappé…');}}
...
// Trajet de voyage : polyligne lissée (Catmull-Rom) échantillonnée à longueur d'arc, passant par les points de passage
// optionnels `dock.via` de chaque île (listés du ponton vers le large ; ex. l'Arche fait passer le voilier sous la voûte).
function buildVoyagePath(pts){const P=[pts[0],...pts,pts[pts.length-1]],out=[];for(let i=1;i<P.length-2;i++){const p0=P[i-1],p1=P[i],p2=P[i+1],p3=P[i+2],n=12;for(let k=0;k<n;k++){const t=k/n,t2=t*t,t3=t2*t;out.push([.5*((2*p1[0])+(-p0[0]+p2[0])*t+(2*p0[0]-5*p1[0]+4*p2[0]-p3[0])*t2+(-p0[0]+3*p1[0]-3*p2[0]+p3[0])*t3),.5*((2*p1[1])+(-p0[1]+p2[1])*t+(2*p0[1]-5*p1[1]+4*p2[1]-p3[1])*t2+(-p0[1]+3*p1[1]-3*p2[1]+p3[1])*t3)]);}}
out.push(P[P.length-1]);const cum=[0];for(let i=1;i<out.length;i++)cum.push(cum[i-1]+Math.hypot(out[i][0]-out[i-1][0],out[i][1]-out[i-1][1]));return{pts:out,cum,len:cum[cum.length-1]};}
function voyagePos(t){const {path}=voyage;if(!path)return[voyage.x,voyage.z];const d=clamp(t,0,1)*path.len;let i=1;while(i<path.cum.length-1&&path.cum[i]<d)i++;const a=path.pts[i-1],b=path.pts[i],seg=path.cum[i]-path.cum[i-1],u=seg>1e-6?(d-path.cum[i-1])/seg:0;return[a[0]+(b[0]-a[0])*u,a[1]+(b[1]-a[1])*u];}
function startVoyage(toId){const from=currentIsland(),to=islands.find(i=>i.id===toId);if(!to||to===from||voyage.active)return;const a=dockSeaEnd(from),b=dockSeaEnd(to),c1=dockOffshore(from),c2=dockOffshore(to);const path=buildVoyagePath([[a[0],a[2]],c1,...(from.dock.via||[]),...[...(to.dock.via||[])].reverse(),c2,[b[0],b[2]]]);
Object.assign(voyage,{active:true,from,to,a,b,c1,c2,path,t:0,dur:Math.max(8,path.len/5.5),x:a[0],z:a[2],heading:from.dock.heading});cam.mode='voyage';promptEl.hidden=true;actionBtn.hidden=true;sfx('cast');toast(`Cap sur ${to.name} !`);dirty=true;}
...
switch(e.code){case'KeyE':case'Enter':interact();e.preventDefault();break;case'Space':jump();e.preventDefault();break;case'Escape':if(!shopEl.hidden){shopEl.hidden=true;saveGame(true);}else if(minigame)cancelFishing();else if(dialog){dialog=null;dialogEl.hidden=true;for(const n of npcs)n.talking=false;}else togglePanel();break;case'KeyH':dbg.hidden=!dbg.hidden;break;case'Digit1':case'Digit2':case'Digit3':case'Digit4':case'Digit5':case'Digit6':{const l=cropList(),i=parseInt(e.code.slice(5))-1;if(i<l.length){game.seedSel=i;toast('Graine sélectionnée : '+CROPS[l[i]].name);}else toast('Pas encore de culture débloquée à cette touche (voir Anaé ou Basile).');break;}case'KeyF':fpsEl.hidden=!fpsEl.hidden;break;case'KeyM':mapEl.hidden=!mapEl.hidden;try{localStorage.setItem('azura-map',mapEl.hidden?'0':'1');}catch(x){}break;case'Equal':case'NumpadAdd':goal.r=clamp(goal.r*.9,4,65);break;case'Minus':case'NumpadSubtract':goal.r=clamp(goal.r*1.1,4,65);break;case'Home':$('#reset').onclick();break;}});
...
if(game.quests.paniers===1)for(const s of panierSpots)if(!game.paniers.has(s.id))dot(s.x,s.z,'#e8a24c',4);
if(game.quests.truites===1&&game.trout<2)dot(gorgeFishSpot.x,gorgeFishSpot.z,'#7ae7ff',5);
const q=game.quests,nd=Object.keys(game.delivered).length;
for(const n of npcs){if(n.cat)continue;const ready={tomas:q.shells===0||(q.shells===1&&game.shells.size>=8)||(q.shells===2&&q.fish===0)||(q.bread===1&&!game.delivered.tomas),anae:q.cat===0||(q.cat===1&&game.catFollow)||(q.fish===1&&game.fish>=3)||(q.laine===0&&q.market===2)||(q.laine===1&&(game.produce.laine||0)>=2),lila:q.crabs===0||(q.crabs===1&&game.crabsCaught>=3),pia:q.bread===0||(q.bread===1&&nd>=3)||(q.bouquet===0&&q.market===2)||(q.bouquet===1&&(game.produce.fleur||0)>=3),marco:q.treasure===0||(q.bread===1&&!game.delivered.marco),bastien:q.notes===0||(q.notes===1&&game.notes.size>=3),oro:(stars()>=TOTAL&&!n.final)||(q.bread===1&&!game.delivered.oro),gardien:q.goats===0||(q.goats===1&&game.goatsCaught>=3)||(q.goats===2&&q.oil===0)||(q.oil===1&&game.oil),marchand:q.farm>=1&&(q.market===0||(q.market===1&&(game.produce.tomate||0)>=3&&(game.produce.ble||0)>=3)),sibylle:(q.farm>=1&&q.farine===0)||(q.farine===1&&game.flour),nino:q.paniers===0||(q.paniers===1&&game.paniers.size>=3),margot:(q.farine===1&&!game.flour&&(game.produce.ble||0)>=3)||(q.fish>=1&&q.truites===0)||(q.truites===1&&game.trout>=2)}[n.id];
...
// ---------- démarrage ----------
function showStart(){const s=loadSave();const hide=(sel,v)=>{const el=document.querySelector(sel);if(el)el.hidden=v;};hide('#loading-title',true);hide('#loading-help',true);hide('.sun',true);if(!$('#start'))return;$('#start').hidden=false;
...
else if(c.state==='idle'){idleEntity(c,dt);c.t-=dt;if(c.t<=0){const a=Math.random()*TAU,r=Math.random()*(c.goat?c.leash:2.2),tx=c.home[0]+Math.cos(a)*r,tz=c.home[1]+Math.sin(a)*r,ti=cellIndex(tx,tz);if(ti<0||!reach[ti]||Math.abs(cellH(tx,tz)-c.y)>1.2){c.t=1;return;}c.target=[tx,tz];c.state='walk';c.t=2+Math.random()*4;}}
...
updateFishing(dt);
waterfallUpdate(player.x,player.z);
...
if(game.quests.paniers===1)for(const s of panierSpots){if(game.paniers.has(s.id))continue;if(Math.hypot(s.x-player.x,s.z-player.z)<.7&&Math.abs(s.y-player.y)<.9){game.paniers.add(s.id);sfx('pick');toast(`🧺 Panier retrouvé ! ${game.paniers.size}/3`);refreshHUD();dirty=true;saveGame(true);}}
...
for(const n of npcs){if(n.cat)poseCat(n,t);else poseHuman(n,t);drawList.push({mesh:n.mesh,bones:n.bones,n:6,mode:4});if(n.id==='gardien'&&typeof gardienLanternDraw==='function')drawList.push(gardienLanternDraw(n));}
...
farmDraw(t);
if(game.quests.oil===2){if(typeof phareLanternDraw==='function'){const g=phareLanternDraw(t,true);if(g)drawList.push(g);}else{setBone(lighthouseBones,0,mm(T(34,12.5,-24.6),SC(3,3,3)));drawList.push({mesh:lanternGlow,bones:lighthouseBones,n:1,mode:7,noShadow:true});}}
...
if(game.quests.paniers===1)spin(panierSpots,panierMesh,game.paniers,.1);
