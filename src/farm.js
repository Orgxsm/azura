...
function farmDraw(t){coopDraw(t);
if(actGesture&&actGesture.th.act==='water'&&typeof waterDropsDraw==='function'){const B=player.bones,o=64,sc=player.scale||1,px=.3*sc,py=.235*sc,pz=.44*sc;const x=B[o]*px+B[o+4]*py+B[o+8]*pz+B[o+12],y=B[o+1]*px+B[o+5]*py+B[o+9]*pz+B[o+13],z=B[o+2]*px+B[o+6]*py+B[o+10]*pz+B[o+14];const d=waterDropsDraw(x,y,z,actGesture.t/.6);if(d)drawList.push(d);}for(const c of farmCells){const p=plotOf(c);if(!p)continue;setBone(c.bones,0,T(c.x,c.y,c.z));drawList.push({mesh:p.wet?soilWetMesh:soilMesh,bones:c.bones,n:1,mode:4,noShadow:true});
...
const shopEl=$('#shop');
let shopBack=null;
function openShop(mode){shopEl.hidden=false;const box=shopEl.querySelector('div');box.innerHTML='';const h=document.createElement('h2');h.textContent=mode==='buy'?(shopBack?'Graines':'Graines d\'Anaé'):mode==='sell'?(shopBack?'Ta récolte':'Comptoir de Pia'):'Étal de Basile';box.appendChild(h);const p=document.createElement('p');p.textContent=`Tu as ${game.coins} pièces.`;box.appendChild(p);
if(mode==='market'){shopBack=null;for(const [m,label] of[['buy','🌱 Acheter des graines et des poules'],['sell','🧺 Vendre ma récolte et mes œufs']]){const b=document.createElement('button');b.textContent=label;b.onclick=()=>{shopBack='market';openShop(m);};box.appendChild(b);}}
