...
else if(mode==='sell'){const bonus=(!shopBack&&typeof marketBonus==='function')?marketBonus():1;if(bonus>1){const bp=document.createElement('p');bp.textContent='🎪 Marché du matin : Pia paie 20 % de plus !';box.appendChild(bp);}
let any=false;for(const id in PRODUCE){const n=game.produce[id]||0;if(!n)continue;any=true;const c=PRODUCE[id];const b=document.createElement('button');const gain=Math.round(n*c.sell*bonus);b.textContent=`Vendre ${n} ${c.name}${n>1&&id!=='ble'?'s':''} → ${gain} pièces`;b.onclick=()=>{game.coins+=gain;game.produce[id]=0;sfx('coin');dirty=true;refreshHUD();openShop('sell',shopBack);};box.appendChild(b);}
