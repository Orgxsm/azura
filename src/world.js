...
const HN=1200,HS=WORLD.size/HN,SEA=.22,STEP=.42;const WCX=WORLD.x0+WORLD.size/2,WCZ=WORLD.z0+WORLD.size/2,WH=WORLD.size/2;
...
function heightAt(x,z){const fx=(x-WORLD.x0)/HS-.5,fz=(z-WORLD.z0)/HS-.5,ix=Math.floor(fx),iz=Math.floor(fz),tx=fx-ix,tz=fz-iz;const g=(a,b)=>H[clamp(b,0,HN-1)*HN+clamp(a,0,HN-1)];return(g(ix,iz)*(1-tx)+g(ix+1,iz)*tx)*(1-tz)+(g(ix,iz+1)*(1-tx)+g(ix+1,iz+1)*tx)*tz;}
// Terrasses pavées d'abord (elles écrasent les bosses, laissent passer sous les balcons), puis escaliers en absolu.
for(const p of platforms)for(let z=p.z-p.r;z<=p.z+p.r;z+=HS)for(let x=p.x-p.r;x<=p.x+p.r;x+=HS){const i=cellIndex(x,z);if(i<0)continue;const px=x-((x-WORLD.x0)%HS)+HS/2,pz=z-((z-WORLD.z0)%HS)+HS/2;if(Math.hypot(px-p.x,pz-p.z)>p.r)continue;const g=H[i];if(g<p.y+1||(g>p.y+1.5&&g<p.y+3.2))H[i]=p.y;}
...
function floodFrom(x,z){const start=cellIndex(x,z);const q=new Int32Array(HN*HN);let qi=0,qn=0;q[qn++]=start;reach[start]=1;while(qi<qn){const i=q[qi++],h=H[i],ix=i%HN,iz=(i-ix)/HN;for(const[dx,dz]of[[1,0],[-1,0],[0,1],[0,-1]]){const nx=ix+dx,nz=iz+dz;if(nx<0||nz<0||nx>=HN||nz>=HN)continue;const j=nz*HN+nx;if(reach[j]||blocked[j])continue;const hj=H[j];if(hj>SEA&&Math.abs(hj-h)<=STEP){reach[j]=1;q[qn++]=j;}}}return qn;}
reach.fill(0);let reachCount=0;for(const isl of islands)reachCount+=floodFrom(isl.spawn[0],isl.spawn[1]);
