...
const src=ctx.createBufferSource();src.buffer=buf;src.loop=true;const lp=ctx.createBiquadFilter();lp.type='lowpass';lp.frequency.value=520;const g=ctx.createGain();g.gain.value=.05;const lfo=ctx.createOscillator();lfo.frequency.value=.11;const lg=ctx.createGain();lg.gain.value=.035;lfo.connect(lg);lg.connect(g.gain);lfo.start();src.connect(lp);lp.connect(g);g.connect(master);src.start();audio.waves=g;
// chute de la Gorge : même bruit, filtré plus clair, avec un léger frémissement ; volume réglé par distance (waterfallUpdate)
const fsrc=ctx.createBufferSource();fsrc.buffer=buf;fsrc.loop=true;const flp=ctx.createBiquadFilter();flp.type='lowpass';flp.frequency.value=1600;const fg=ctx.createGain();fg.gain.value=0;const flfo=ctx.createOscillator();flfo.frequency.value=2.7;const flg=ctx.createGain();flg.gain.value=.012;flfo.connect(flg);flg.connect(fg.gain);flfo.start();fsrc.connect(flp);flp.connect(fg);fg.connect(master);fsrc.start();audio.fall=fg;}catch(e){console.warn('Audio indisponible',e);}}
// Volume de la chute selon la distance au segment de la cascade de la Gorge (x [-1.5,1.5], z [31.6,34.4]).
function waterfallUpdate(px,pz){if(!audio.ctx||!audio.fall)return;const x=Math.max(-1.5,Math.min(1.5,px)),z=Math.max(31.6,Math.min(34.4,pz));const d=Math.hypot(px-x,pz-z);const v=Math.max(0,1-d/24);audio.fall.gain.setTargetAtTime(v*v*.22,audio.ctx.currentTime,.25);}
...
function musicTick(){const ctx=audio.ctx;if(!ctx||audio.muted||!music.on)return;const night=(typeof dayF==='number')?1-dayF:0;const fete=(typeof feteActive!=='undefined'&&feteActive)?1:0;const beat=60/(74-night*16+fete*12);
