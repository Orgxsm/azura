// islands/phare.js — Île du Phare · Astra · passe 3, vires et port habité
// Base publique : 510b5d2. Coordonnées en mètres, Y vertical, mer Y=0.
  ...
  try {
    ...
    function surfaceY(x,z,r) {
      ...
      for(const level of[1.65,5.15,8.55]){
        ...
        height-=d*(1-smooth((Math.abs(d)-.36)/.58))*sculpt;
      }
    ...
    seed = 341810;
    const leafBase=color(0x638244),leafSun=color(0x7a9e56),leafShade=color(0x4e6b36);
    const bark=color(0x7b634b),rope=color(0xbba780);
    ...
    // Accessoires fixés à la façade +X de la maison basse. Toute leur géométrie
    // reste dans l'emprise DÉJÀ bloquée de cette maison (marge incluse).
    const facadeBounds=[];
    {
      const pos=[40,3.6,-12.8],rot=-.05,P=p=>transform(p,pos,rot);
      const start=verts.length,count=triangles,shutter=color(0x526b87);
      for(const side of[-1,1]){
        box(P([1.47,1.26,side*.46]),[.026,.73,.18],shutter,rot);
        for(let j=0;j<5;j++)box(P([1.489,.96+j*.145,side*.46]),[.016,.035,.18],tint(shutter,1.15),rot);
      }
      // Treille décalée vers l'arrière, laissant la fenêtre visible.
      for(let j=0;j<4;j++){
        cylinder(P([1.29,.28,-.91+j*.14]),P([1.29,1.75,-.91+j*.14]),.012,.012,palette.trim,5);
        cylinder(P([1.29,.45+j*.36,-.94]),P([1.29,.45+j*.36,-.43]),.012,.012,palette.trim,5);
      }
      cylinder(P([1.34,0,-.69]),P([1.34,.21,-.69]),.065,.10,palette.tile,10);
      cylinder(P([1.34,.19,-.69]),P([1.34,.225,-.69]),.108,.108,tint(palette.tile,1.12),10);
      for(let j=0;j<7;j++){
        const z=-.7+.16*Math.sin(j*2.2),y=.37+j*.20;
        leaves(P([1.32,y,z]),[.045,.13,.14],j%2?leafBase:leafSun);
        if(j%2===0)for(let k=0;k<3;k++)ellipsoid(P([1.37,y+.02+(k%2)*.045,z-.045+k*.04]),[.025,.037,.035],color(0xb9768e),6,3,0);
      }
      let valid=true;
      for(let i=start;i<verts.length;i+=9){
        const dx=verts[i]-pos[0],dz=verts[i+2]-pos[2];
        const x=dx*Math.cos(rot)-dz*Math.sin(rot),z=dx*Math.sin(rot)+dz*Math.cos(rot);
        if(Math.abs(x)>1.54||Math.abs(z)>1.34)valid=false;
      }
      if(!valid){verts.length=start;triangles=count;}
      else facadeBounds.push({first:start,end:verts.length});
    }
  } finally {
