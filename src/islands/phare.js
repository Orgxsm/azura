// islands/phare.js — Île du Phare · Astra · passe 2, calcaire et jardins
// Base publique : 36e90af. Coordonnées en mètres, Y vertical, mer Y=0.
...
// de la session de préparation ne dispose pas de WebGL 2.
{
  ...
  try {
    ...
    function material(p,r,up=1) {
      ...
      const stone=tint(color(0xcbb88a),band),sand=color(0xd6c699),turf=color(0x82935c);
      const shore=(1-smooth((p[1]-.48)/1.15))*Math.max(smooth((p[2]+12)/2),smooth((r-.86)/.10));
      const cover=smooth(gardenCover(p[0],p[2])*2.2)*smooth((up-.55)/.4);
      return paint(paint(stone,turf,cover*.78),sand,shore);
    }
    function terrainNormal(p) {
      const d=.06;
      return norm([terrainAt(p[0]-d,p[2])-terrainAt(p[0]+d,p[2]),2*d,
        terrainAt(p[0],p[2]-d)-terrainAt(p[0],p[2]+d)]);
    }
    function paintedTri(a,b,c,colors,normals) {
      const start=verts.length;tri(a,b,c,colors[0],normals);
    ...
    const normals=rings.map(row=>row.map(terrainNormal));
