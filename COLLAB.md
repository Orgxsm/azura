...
Dépôt Git : `/Users/jl/azura` sur l'ordinateur de Léo, remote GitHub public `https://github.com/Orgxsm/azura` (origin). On pousse ses branches ; `main` reste la branche d'intégration.
...
- **Eau courante** : `riverData` (scene_end.js, Claude) = nappes d'eau dessinées par le shader d'eau avec `uFlow` ; la composante rouge de la couleur d'un sommet encode l'écume (0,08 = eau calme, 0,7 = bassin bouillonnant, 1 = chute). Une île qui veut une rivière fournit le profil de fond dans ses notes (comme PASSE-10A.md), Claude pose l'eau.
- **Points de passage du voilier** : dans le registre `archipel.js`, `dock.via:[[x,z],…]` (optionnel) liste, du ponton vers le large, les points par lesquels le voilier passe avant de rejoindre l'autre île (trajet Catmull-Rom à vitesse constante, 5,5 m/s). Astra les propose dans l'en-tête de son île (chenal, contournement d'un écueil), Claude les inscrit.
- **Ponts de corde** : `ropeBridge(points, width=1.2, {sag:.055, rail:true, plank:.3, posts:true})` (util.js, Claude) rend planches, cordes porteuses, mains courantes, suspentes et poteaux, et déclare le tablier affaissé dans `stairDefs` (hauteurs absolues). Les appuis doivent être sur des cellules praticables ; la flèche vaut `sag × portée` au milieu. Retourne la polyligne affaissée.
