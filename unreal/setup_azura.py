...
# ---------- 1. import des .glb ----------
def pipeline_couleurs_sommet():
    """Interchange ignore COLOR_0 par defaut : pipeline force en IVCIO_REPLACE."""
    p = unreal.InterchangeGenericAssetsPipeline()
    p.common_meshes_properties.set_editor_property('vertex_color_import_option', unreal.InterchangeVertexColorImportOption.IVCIO_REPLACE)
    o = unreal.InterchangePipelineStackOverride()
    o.add_pipeline(p)
    return o
...
def import_glb(name):
    ...
    task.set_editor_property('options', pipeline_couleurs_sommet())
...
def vertex_color_material():
    ...
    lib.connect_material_property(vc, '', unreal.MaterialProperty.MP_BASE_COLOR)  # la sortie couleur s'appelle '' (pas 'RGB')
...
def detect_mapping(mesh, bounds_json):
    """Déduit comment Unreal a placé les axes glTF (x, y-haut, z) : renvoie une fonction (x,y,z) mètres -> Vector cm.
    Bounds pris sur le maillage construit (pas l'acteur, dont les bounds peuvent ne pas être prêts),
    et chaque axe Unreal ne peut être apparié qu'une fois."""
    b = mesh.get_bounds()
    ...
    mapping = {}
    pris = set()
    for k in sorted(range(3), key=lambda k: -gl_ext[k]):  # axes les plus discriminants d'abord
        j = min((i for i in range(3) if i not in pris), key=lambda i: abs(ext[i] - gl_ext[k]))
        pris.add(j)
...
def main():
    ...
    for entry in SCENE['meshes']:
        ...
        for m in meshes:
            ...
            if entry['id'] == 'azura' and conv is None:
                # bornes glTF d'Azura (union des accessors POSITION de TOUTES les primitives) pour calibrer les axes
                import struct
                ...
                conv = detect_mapping(m, {'min': mn, 'max': mx})
    ...
    # lumière, ciel, brume
    sun = unreal.EditorLevelLibrary.spawn_actor_from_class(unreal.DirectionalLight, unreal.Vector(0, 0, 3000), unreal.Rotator(0, -42, 30))  # (roll, pitch, yaw) : soleil plongeant
    sun.set_actor_label('Azura_Soleil'); sun.tags = ['azura']
    sun.light_component.set_intensity(10.0); sun.light_component.set_light_color(unreal.LinearColor(1.0, 0.94, 0.82))
    ...
    # skylight AU-DESSUS des iles : en capture temps reel, placee dans un maillage elle capture du noir
    skl = unreal.EditorLevelLibrary.spawn_actor_from_class(unreal.SkyLight, unreal.Vector(0, 0, 15000)); skl.set_actor_label('Azura_LumiereCiel'); skl.tags = ['azura']
    skl.light_component.set_editor_property('real_time_capture', True)
    skl.light_component.set_editor_property('intensity', 2.0)
    skl.light_component.set_editor_property('lower_hemisphere_is_black', False)  # rebond de la mer
    ...
    fog.component.set_editor_property('fog_density', 0.0015); fog.component.set_editor_property('fog_inscattering_luminance', unreal.LinearColor(0.78, 0.86, 0.95))
    # exposition : le style pastel sature vite, leger biais negatif global
    ppv = unreal.EditorLevelLibrary.spawn_actor_from_class(unreal.PostProcessVolume, unreal.Vector(0, 0, 0))
    ppv.set_actor_label('Azura_Expo'); ppv.tags = ['azura']; ppv.set_editor_property('unbound', True)
    ps = ppv.get_editor_property('settings')
    ps.set_editor_property('override_auto_exposure_bias', True); ps.set_editor_property('auto_exposure_bias', -0.75)
