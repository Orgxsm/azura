...
function Run-Py($script) {
  Write-Host "== $script"
  $eap = $ErrorActionPreference; $ErrorActionPreference = "Continue"  # l'editeur ecrit du bruit CEF sur stderr : ne pas en faire une erreur fatale
  & $Cmd $Proj -ExecutePythonScript="$Root\unreal\$script" -unattended -nosplash -log 2>&1 | ForEach-Object { "$_" } | Select-String -Pattern "\[Azura\]|Error|Warning: Script" | Select-Object -Last 40
...
if ($Step -in "all","render") {
  ...
  & $Cmd $Proj /Game/Azura/Maps/Vitrine -game -LevelSequence=/Game/Azura/LS_AzuraVitrine -MoviePipelineConfig=/Game/Azura/MRQ_Vitrine -windowed -ResX=1280 -ResY=720 -NoLoadingScreen -log 2>&1 | ForEach-Object { "$_" } | Select-String -Pattern "MoviePipeline|Error" | Select-Object -Last 20
  $ErrorActionPreference = $eap
}
